...
const fetcher = (newsId: number, body: string) => axios.get(SERVER_URL + API_URL + `/${newsId}` + body).then(({ data }) => data);
 ...
 */
const useMainNewsDetail = (newsId: number, categoryId: number) => {
    const isLogin = useRecoilValue(LoginState)[0]
    let body: string
    if (isLogin.id) {
        body = `/${categoryId}/${isLogin.id}`
    } else {
        body = `/${categoryId}`
    }
    ...
    return useQuery([MAIN_NEWS_DETAIL], () => fetcher(newsId, body), {
