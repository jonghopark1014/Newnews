...
const fetcher = (newsId: number, categoryId: number) => axios.get(SERVER_URL + API_URL + `/${newsId}/${categoryId}`).then(({ data }) => data);
...
const useMainNewsDetail = (newsId: number, categoryId: number) => {
    ...
    return useQuery([MAIN_NEWS_DETAIL], () => fetcher(newsId, categoryId), {
